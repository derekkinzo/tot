    ...
    <div style={{ position: 'relative', display: 'inline-block' }}>
      ...
      {open && (
        <div style={{
          ...
          {loading && <div style={{ padding: '10px 14px', color: TEXT.secondary, fontSize: 12 }}>Loading…</div>}
          {!loading && error && <div style={{ padding: '10px 14px', color: '#f87171', fontSize: 12 }}>Failed to load sessions.</div>}
          {!loading && !error && sessions.length === 0 && <div style={{ padding: '10px 14px', color: TEXT.secondary, fontSize: 12 }}>No sessions.</div>}
          {!loading && !error && sessions.map((s) => (
            <button
              ...
              style={{
                ...
                border: 'none', color: TEXT.primary, fontSize: 13,
